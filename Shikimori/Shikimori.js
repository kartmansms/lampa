(function () {
    ...
    const ShikimoriConfig = window.ShikimoriConfig || {
        api: {
            shikimori: 'https://shikimori.one/api/graphql',
            animeapi: {
                ids: 'https://arm.haglund.dev/api/v2/ids',
                themoviedb: 'https://arm.haglund.dev/api/v2/themoviedb'
            },
            tmdb: 'https://api.themoviedb.org/3/',
            tmdbProxy: function() {
                const domain = Lampa.Manifest?.cub_domain || 'cub.red';
                return `apitmdb.${domain}/3/`;
            }
        },
        tmdbApiKey: window.shikimoriTMDBApiKey || "4ef0d7355d9ffb5151e987764708ce96",
        defaults: {
            itemsPerPage: 36,
    ...
    const Cache = {
        ...
        set(key, value, ttl = ShikimoriConfig.defaults.cacheTTL) {
            const item = {
                value,
                expiry: Date.now() + ttl
            };
    ...
    // ОБРАБОТКА ЗАПРОСОВ С КЭШИРОВАНИЕМ И ПОВТОРАМИ
    const RequestManager = {
        async request(url, options = {}) {
            const cacheKey = this._generateCacheKey(url, options);
            const useCache = options.method === 'GET' && 
                           ShikimoriConfig.features.cacheEnabled &&
                           !options.noCache;
            ...
            // Выполняем запрос
            try {
                const data = await this._executeRequest(url, options);
        ...
        _executeRequest(url, options, retries = ShikimoriConfig.features.retryEnabled ? 3 : 0) {
            return new Promise((resolve, reject) => {
                const ajaxOptions = {
                    url: url,
                    method: options.method || 'GET',
                    timeout: options.timeout || ShikimoriConfig.defaults.timeout,
                    ...
                    error: (xhr, status, error) => {
                        if (retries > 0 && this._isRetryableError(xhr.status)) {
                            console.log(`Retrying request... (${retries} attempts left)`);
                            setTimeout(() => {
                                this._executeRequest(url, options, retries - 1)
                                    .then(resolve)
                                    .catch(reject);
                            }, 1000);
                        } else {
    ...
    function main(params, oncomplete, onerror) {
        $(document).ready(function () {
            ...
            // AJAX-запрос к API Shikimori с использованием RequestManager
            RequestManager.request(ShikimoriConfig.api.shikimori, {
                method: 'POST',
                data: JSON.stringify({
                    query: query
                }),
                contentType: 'application/json',
                noCache: true // GraphQL запросы не кэшируем
            })
            .then(function(response) {
                oncomplete(response.data.animes);
            })
            .catch(function(error) {
                console.error('Ошибка Shikimori API:', error);
                onerror(error);
            });
        });
    }
    ...
    function search(animeData) {
        Lampa.Noty.show('Поиск информации...', 0);
        ...
        // Первый GET запрос к https://arm.haglund.dev/api/v2/ids с кэшированием
        RequestManager.request(ShikimoriConfig.api.animeapi.ids, {
            data: {
                source: 'myanimelist',
                id: animeData.id
            },
        ...
        .catch(function(error) {
            if (error.xhr && error.xhr.status === 404) {
                ...
                console.error('Ошибка при получении данных с animeapi:', error);
                searchTmdb(animeData.name, function (tmdbResponse) {
                    handleTmdbResponse(tmdbResponse, animeData.japanese);
                });
            }
        ...
        function searchTmdb(query, callback) {
            const apiKey = ShikimoriConfig.tmdbApiKey;
            const apiUrlTMDB = ShikimoriConfig.api.tmdb;
            const apiUrlProxy = ShikimoriConfig.api.tmdbProxy();
            const request = "search/multi?api_key=".concat(apiKey, "&language=").concat(Lampa.Storage.field('language'), "&include_adult=true&query=").concat(encodeURIComponent(cleanName(query)));
            
            const url = Lampa.Storage.field('proxy_tmdb') ? 
        ...
        function getTmdb(id, type = 'movie', callback) {
            ...
            const apiUrlTMDB = ShikimoriConfig.api.tmdb;
            const apiUrlProxy = ShikimoriConfig.api.tmdbProxy();
            const request = "".concat(type, "/").concat(id, "?api_key=").concat(apiKey, "&language=").concat(Lampa.Storage.field('language'));
    ...
    function Component$1(object) {
        ...
        var head = $("<div class='Shikimori-head torrent-filter'><div class='Shikimori__home simple-button simple-button--filter selector'>Главная</div><div class='Shikimori__search simple-button simple-button--filter selector'>Фильтр</div><div class='Shikimori__clear-cache simple-button simple-button--filter selector'>Очистить кэш</div></div>");
        ...
        this.headeraction = function () {
            // Загрузка жанров с кэшированием
            RequestManager.request(ShikimoriConfig.api.shikimori.replace('/graphql', '/genres'), {
                timeout: 10000
            })
            .then(function(response) {
    ...
    return seasons;
            ...
            var clearCacheElement = head.find('.Shikimori__clear-cache');
            ...
            clearCacheElement.on('hover:enter', function () {
                RequestManager.clearCache();
                Lampa.Noty.show('Кэш очищен', 2000);
            });
            ...
            return _regeneratorRuntime().wrap(function _callee$(_context) {
                while (1) switch (_context.prev = _context.next) {
                    case 0:
                        ...
                        _context.prev = 1;
                        _context.next = 4;
                        return RequestManager.request(ShikimoriConfig.api.animeapi.themoviedb, {
                            data: { id: e.object.id },
                            timeout: 10000
                        });
                    case 4:
                    ...
                    case 8:
                        _context.next = 10;
                        return RequestManager.request(`https://shikimori.one/api/animes/${getMAL[0].myanimelist}`, {
                            timeout: 10000
                        });
                    case 10:
    ...
    function startPlugin() {
        ...
        // Экспорт API для отладки
        window.ShikimoriAPI = API;
        window.ShikimoriCache = Cache;
        window.ShikimoriRequestManager = RequestManager;
    }
