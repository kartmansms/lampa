(function () {
    ...
    // Вспомогательная функция для получения всех ключей объекта
    ...
    // Основная функция для выполнения GraphQL-запроса
    function main(params, oncomplite, onerror) {
        $(document).ready(function () {
            var query = "\n                query Animes {\n                    animes(limit: 36, order: ".concat(params.sort || 'aired_on', ", page: ").concat(params.page || 1, "\n            ");
            if (params.kind) {
            ...
            // Исправление: заменил params.seasons на params.season
            if (params.season) {
                query += ", season: \"".concat(params.season, "\"");
            }
            query += ") {\n                        id\n                        name\n                        russian\n                        licenseNameRu\n                        english\n                        japanese\n                        kind\n                        score\n                        status\n                        season\n                        airedOn { year }\n                        poster {\n                            originalUrl\n                        }\n                    }\n                }\n            ";
    ...
    function search(animeData) {
        ...
        $.get("https://arm.haglund.dev/api/v2/ids?source=myanimelist&id=".concat(animeData.id), function (response) {
            ...
            if (response && response.themoviedb) {
                console.log('Используем TMDB ID:', response.themoviedb);
                getTmdb(response.themoviedb, animeData.kind, processResults);
            } else {
                console.log('Поиск по названию:', animeData.name);
                searchTmdb(cleanName(animeData.name), function (tmdbResponse) {
                    if (tmdbResponse.total_results === 0 && animeData.japanese) {
                        ...
                        searchTmdb(animeData.japanese, processResults);
                    } else {
            ...
            console.warn('Ошибка при запросе к animeapi.my.id:', jqXHR.status);
            searchTmdb(cleanName(animeData.name), processResults;
        });

        function searchTmdb(query, callback) {
            if (!query) {
                console.warn('Пустой запрос для TMDB');
                return;
            }
            ...
            var request = "search/multi?api_key=".concat(apiKey, "&language=").concat(Lampa.Storage.field('language') || 'ru', "&include_adult=true&query=").concat(encodeURIComponent(query));
        ...
        function getTmdb(id, type, callback) {
            if (!id) {
                console.warn('Отсутствует TMDB ID');
                return;
            }
            ...
            var apiUrlTMDB = 'https://api.themoviedb.org/3/';
            var apiUrlProxy = 'apitmdb.' + (Lampa.Manifest && Lampa.Manifest.cub_domain ? Lampa.Manifest.cub_domain : 'cub.red') + '/3/';
            var request = "".concat(type, "/").concat(id, "?api_key=").concat(apiKey, "&language=").concat(Lampa.Storage.field('language') || 'ru');
            var url = Lampa.Storage.field('proxy_tmdb') ? Lampa.Utils.protocol() + apiUrlProxy + request : apiUrlTMDB + request;
            ...
            $.get(url, function (response) {
                if (response && response.id) {
                    ...
                    console.warn('TMDB: Контент не найден');
                }
            }).fail(function (jqXHR) {
        ...
        function processResults(response) {
            if (!response) {
                console.warn('Нет результатов для обработки');
                return;
            }
            ...
            if (response.total_results !== undefined) {
                if (response.total_results === 0) {
                    ...
                    openCard(response.results[0]);
                } 
                else {
                    response.results.forEach(function (animeItem) {
                        menu.push({
                            title: "[".concat(animeItem.media_type.toUpperCase(), "] ").concat(animeItem.name || animeItem.title),
                            card: animeItem
                        });
                    });
        ...
        function openCard(card) {
            ...
            Lampa.Activity.push({
                ...
                component: 'full',
                id: card.id,
                method: card.number_of_episodes ? 'tv' : 'movie',
                card: card
            });
        }
    }
    ...
    function Card(data, userLang) {
        ...
        var statusTranslations = {
            ...
            'ongoing': 'Онгоинг',
            'released': 'Вышло',
            'default': data.status || 'Неизвестно'
        };

        // Исправление: обработка отсутствия данных о сезоне
        var seasonText = '';
        if (data.season) {
            seasonText = data.season.replace(/_/g, ' ')
        ...
        else if (data.airedOn && data.airedOn.year) {
            seasonText = data.airedOn.year + ' год';
        }

        // Приоритет названий
        var title = data.licenseNameRu || 
                   (userLang === 'ru' ? data.russian : null) || 
                   data.name || 
                   data.japanese || 
                   'Без названия';

        // Исправление: обработка отсутствующего постера
        var posterUrl = data.poster && data.poster.originalUrl 
            ? data.poster.originalUrl 
            : 'https://via.placeholder.com/200x300?text=No+poster';

        var item = Lampa.Template.get("Shikimori-Card", {
            img: posterUrl,
            type: typeTranslations[data.kind] || typeTranslations['default'],
            status: statusTranslations[data.status] || statusTranslations['default'],
            rate: data.score || '?',
            title: title,
            season: seasonText
        });
    ...
    function Component$1(object) {
        var userLang = Lampa.Storage.field('language') || 'ru';
        ...
        this.headeraction = function () {
            ...
            // Сезоны
            filters.season = { // Исправление: переименовано seasons -> season
                title: 'Сезон',
            ...
            function generateSeasonJSON() {
                ...
                return [
                    { code: 'winter_2023', title: 'Зима 2023' },
                    { code: 'spring_2023', title: 'Весна 2023' },
                    // ... другие сезоны ...
                ];
            }
            ...
            function showFilterMenu() {
                for (var i in filters) {
                    if (filters[i].items) {
                        filters[i].subtitle = filters[i].items
            ...
            function showSubMenu(filter) {
                Lampa.Select.show({
                    ...
                    onSelect: function(item) {
                        // Сбрасываем выбор для радио-фильтров
                        if (['status', 'AnimeKindEnum', 'sort', 'season'].includes(Object.keys(filters).find(k => filters[k] === filter))) {
                            filter.items.forEach(i => i.selected = false);
                        }
            ...
            function applyFilters() {
                ...
                if (filters.AnimeKindEnum.items.some(i => i.selected)) {
                    params.kind = filters.AnimeKindEnum.items.find(i => i.selected).code;
                }
                
                if (filters.status.items.some(i => i.selected)) {
                    params.status = filters.status.items.find(i => i.selected).code;
                }
                
                if (filters.kind.items.some(i => i.selected)) {
                    params.genre = filters.kind.items.find(i => i.selected).id;
                }
                
                if (filters.sort.items.some(i => i.selected)) {
                    params.sort = filters.sort.items.find(i => i.selected).code;
                }
                
                if (filters.season.items.some(i => i.selected)) {
                    params.season = filters.season.items.find(i => i.selected).code;
                }
        ...
        this.destroy = function () {
            network.clear();
            items.forEach(item => item.destroy && item.destroy());
            scroll.destroy();
            html.remove();
        };
    }

    // Компонент для расширения информации
    function Component() {
        Lampa.Listener.follow("full", function (e) {
            if (e.type !== "complite") return;
            
            try {
                $.get("https://arm.haglund.dev/api/v2/themoviedb?id=" + e.object.id, function (getMAL) {
                    if (!getMAL || !getMAL.length || !getMAL[0].myanimelist) return;
                    
                    $.get("https://shikimori.one/api/animes/" + getMAL[0].myanimelist, function (response) {
                        if (!response) return;
                        
                        // Добавляем информацию о фанабберах
                        if (response.fandubbers && response.fandubbers.length) {
                            e.object.activity.render().find(".full-descr__right").append(
                                `<div class="full-descr__info">
                                    ...
                                    <div class="full-descr__text">${response.fandubbers.join(', ')}</div>
                                </div>`
                            );
                        }
                        ...
                        if (response.score) {
                            e.object.activity.render().find(".full-start-new__rate-line").prepend(
                                `<div class="full-start__rate rate--shikimori">
                ...
                console.warn('Ошибка при получении доп. информации:', error);
            }
        });
    }
